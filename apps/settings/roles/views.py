...
def role_create(request):
    if request.method == 'POST':
        ...
        if not name:
            ...
            return render(request, 'settings/roles/role_form.html', {'role': None})
...
def role_permissions(request, role_id):
    ...
    # Build dependency map: code → [dependency codes] (for JS auto-select)
    dep_map = {p.code: [d.code for d in p.depends_on.all()] for p in all_perms}
    # Reverse map: code → [dependent codes] (for JS warn-on-uncheck)
    rev_map = {}
    ...
    if request.method == 'POST':
        ...
        RBACService.assign_permissions(role, selected)
